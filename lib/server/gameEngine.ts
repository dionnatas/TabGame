...
function applyTileEffect(playerId: string) {
  const state = getState();
  const player = assertPlayer(playerId);
  ...
  if (tile.type === "luck" || tile.type === "bad_luck") {
    const message = triggerRandomEvent(state, tile.type, playerId);
    addEvent("event_triggered", message);
  }
